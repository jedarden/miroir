#!/usr/bin/env bash
# miroir Marathon Launcher — claude-code @ GLM-4.7 via ZAI proxy
#
...
#
# Usage:
#   ./.marathon/start.sh                 # session "miroir-marathon"
...
SESSION_NAME="${1:-miroir-marathon}"
...
if pgrep -f "needle run --workspace $REPO_DIR" >/dev/null 2>&1; then
    echo "Error: a NEEDLE worker is running against $REPO_DIR (identifier 'bravo')." >&2
    echo "       Marathon + NEEDLE share one git worktree → contention." >&2
    echo "       Stop it first:  needle stop -i bravo" >&2
    exit 1
fi
...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║          miroir Marathon — claude-code @ GLM-4.7            ║"
echo "╚══════════════════════════════════════════════════════════════╝"
echo "  Repo:        $REPO_DIR"
