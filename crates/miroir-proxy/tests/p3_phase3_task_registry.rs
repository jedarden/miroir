...
fn test_sqlite_all_14_tables_initialized() {
    ...
    store
        .upsert_node_settings_version("idx1", "node-0", 1, now_ms())
        .expect("Should upsert settings version");
    assert!(store
        .get_node_settings_version("idx1", "node-0")
        .expect("Should get settings version")
        .is_some());
    ...
    store
        .try_acquire_leader_lease("scope1", "pod1", now_ms() + 10000, now_ms())
        .expect("Should acquire lease");
    assert!(store
        .get_leader_lease("scope1")
    ...
    store
        ...
        .expect("Should insert canary run");
    let runs = store
        .get_canary_runs("canary1", 10)
        .expect("Should get runs");
    ...
    assert!(store
        .get_rollover_policy("policy1")
        .expect("Should get policy")
        .is_some());
...
fn test_alias_upsert_and_list() {
    ...
    // Upsert by flipping
    store
        .flip_alias("write-alias", "logs-2025-01-02", 10)
        .expect("Should flip alias");

    let retrieved = store
        .get_alias("write-alias")
        .expect("Should get alias")
    ...
    assert!(store
        .get_alias("write-alias")
        .expect("Should get write alias")
        .is_some());
    assert!(store
        .get_alias("read-alias")
        .expect("Should get read alias")
        .is_some());
}
...
fn test_job_list_by_state() {
    ...
    let queued = store
        ...
        .expect("Should list queued jobs");
    assert_eq!(queued.len(), 1);
    assert_eq!(queued[0].id, "job-0");

    let in_progress = store
        .list_jobs_by_state("in_progress")
        .expect("Should list in-progress jobs");
...
fn test_task_survives_store_reopen() {
    ...
    // Reopen the store (simulating pod restart)
    {
        ...
        store
            .migrate()
            .expect("Migration should succeed after reopen");

        let task = store
            .get_task("persistent-task")
...
fn test_all_tables_survive_store_reopen() {
    ...
    // Write to all 14 tables
    {
        ...
        store
            .upsert_node_settings_version("i1", "n0", 1, now_ms())
            .expect("Should upsert settings version");
    ...
    // Reopen and verify all data
    {
        ...
        assert!(store
            .get_node_settings_version("i1", "n0")
            .expect("Should get settings version")
            .is_some());
        ...
        assert!(store
            ...
            .is_some());
        assert!(store
            .get_idempotency_entry("k1")
            .expect("Should get idempotency entry")
            .is_some());
        ...
        assert!(!store
            .get_canary_runs("c1", 10)
            .expect("Should get canary runs")
            .is_empty());
        assert!(store
            .get_cdc_cursor("sink1", "i1")
            .expect("Should get CDC cursor")
            .is_some());
        assert!(store
            .get_tenant_mapping(&[1])
            .expect("Should get tenant mapping")
            .is_some());
        assert!(store
            .get_rollover_policy("p1")
            .expect("Should get rollover policy")
            .is_some());
        assert!(store
            .get_search_ui_config("i1")
            .expect("Should get search UI config")
            .is_some());
        assert!(store
            .get_admin_session("as1")
            .expect("Should get admin session")
            .is_some());
    }
}
...
fn test_prune_tasks_removes_old_terminal_tasks() {
    ...
    let pruned = store
        .prune_tasks(now - 3600000, 100)
        .expect("Should prune tasks");
    ...
    assert!(store
        .get_task("old-task")
        .expect("Should get old task")
        .is_none());
    assert!(store
        .get_task("recent-task")
        .expect("Should get recent task")
        .is_some());
    assert!(store
        .get_task("active-task")
        .expect("Should get active task")
        .is_some());
}
